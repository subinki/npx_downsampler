...
StackType_t *pxPortInitialiseStack( StackType_t *pxTopOfStack, TaskFunction_t pxCode, void *pvParameters )
{
	...
	*pxTopOfStack = (portSTACK_TYPE)pxCode;			/* Start address */
	...
	pxTopOfStack--;
	*pxTopOfStack = MSTATUS_INIT | read_csr(mstatus);	// hujang
...
// WARNING: the below functions may not have functions

/*Entry Point for Machine Timer Interrupt Handler*/
void vPortSysTickHandler(){
	clear_csr(mie, MIP_MTIP); // disallow_interrupt_timer();
	...
	set_csr(mie, MIP_MTIP); // allow_interrupt_timer();
}
/*-----------------------------------------------------------*/


void vPortSetupTimer()	{
	const unsigned int interval = (configRTC_CLOCK_HZ/configTICK_RATE_HZ);
	clear_csr(mie, MIP_MTIP); // disallow_interrupt_timer();
	_config_timer_interval(interval);
