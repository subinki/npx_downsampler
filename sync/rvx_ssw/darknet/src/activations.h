...
unsigned char quantized_activate(unsigned char x, ACTIVATION a, int quantization_layer_zeropoint);
void activate_quantized_array(unsigned char *x, const int n, const ACTIVATION a, int quantization_layer_zeropoint);
static inline unsigned char quantized_linear_activate(unsigned char x){return x;}
static inline unsigned char quantized_relu_activate(unsigned char x, int quantization_layer_zeropoint){return (x>quantization_layer_zeropoint) ? x : (unsigned char)quantization_layer_zeropoint;}
static inline unsigned char quantized_leaky_activate(unsigned char x){return (x>0) ? x : (unsigned char).1f*x;}
