...
texpar_list_t *get_paths(char *filename)
{
    char *path;
    FILE *file = fopen(filename, "r");
    if(!file) file_error(filename);
    texpar_list_t *lines = make_list();
    while((path=ffgetline(file))){
...
box_label *read_boxes(char *filename, int *n)
{
    ...
    if (!file) {
        ...
        //file_error(filename);
        FILE* fw = fopen("bad.texpar_list_t", "a");
    ...
    int net_w, int net_h)
{
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (id >= classes) {
            ...
            sprintf(buff, "echo %s \"Wrong annotation: class_id = %d. But class_id should be [from 0 to %d]\" >> bad_label.texpar_list_t", labelpath, id, (classes-1));
            system(buff);
            getchar();
            ++sub;
            continue;
        ...
        if ((w < lowest_w || h < lowest_h)) {
            //sprintf(buff, "echo %s \"Very small object: w < lowest_w OR h < lowest_h\" >> bad_label.texpar_list_t", labelpath);
            //system(buff);
            ++sub;
            continue;
        }
        if (x == 999999 || y == 999999) {
            ...
            sprintf(buff, "echo %s \"Wrong annotation: x = 0 or y = 0\" >> bad_label.texpar_list_t", labelpath);
            system(buff);
            ++sub;
            if (check_mistakes) getchar();
            continue;
        ...
        if (x <= 0 || x > 1 || y <= 0 || y > 1) {
            ...
            sprintf(buff, "echo %s \"Wrong annotation: x = %f, y = %f\" >> bad_label.texpar_list_t", labelpath, x, y);
            system(buff);
            ++sub;
            if (check_mistakes) getchar();
            continue;
        ...
        if (w > 1) {
            ...
            sprintf(buff, "echo %s \"Wrong annotation: w = %f\" >> bad_label.texpar_list_t", labelpath, w);
            system(buff);
            w = 1;
            if (check_mistakes) getchar();
        }
        if (h > 1) {
            ...
            sprintf(buff, "echo %s \"Wrong annotation: h = %f\" >> bad_label.texpar_list_t", labelpath, h);
            system(buff);
            h = 1;
            if (check_mistakes) getchar();
        }
...
{
    texpar_list_t *plist = get_paths(filename);
