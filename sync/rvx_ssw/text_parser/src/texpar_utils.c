...
void texpar_strip_char(char *s, char bad)
{
    size_t i;
    size_t len = strlen(s);
    size_t offset = 0;
...
void shuffle(void *arr, size_t n, size_t size)
{
    ...
    for(i = 0; i < n-1; ++i){
        size_t j = i + random_gen()/(32767 / (n-i)+1);
...
void replace_image_to_label(const char* input_path, char* output_path)
{
    ...
    if(strlen(output_path) > 4) {
        ...
        if( strcmp(".txt", output_path_ext) != 0){
            printf("Failed to infer label file name (check image extension is supported): %s \n", output_path);
        }
    }else{
        printf("Label file name is too short: %s \n", output_path);
    }
}
...
texpar_list_t *split_str(char *s, char delim)
{
    size_t i;
    size_t len = strlen(s);
...
texpar_list_t *parse_csv_line(char *line)
{
    texpar_list_t *l = make_list();
    char *c, *p;
    int in = 0;
...
float rand_normal()
{
    ...
    rand1 = random_gen() / ((double) 32767);
    if(rand1 < 1e-100) rand1 = 1e-100;
    rand1 = -2 * log(rand1);
    rand2 = (random_gen() / ((double)32767)) * 2.0 * M_PI;
...
/*
   ...
   int i;
   float sum= 0;
   for(i = 0; i < n; ++i) sum += (float)random_gen()/32767;
   return sum-n/2.;
   }
 */
...
#if (32767 < 65536)
        int rnd = rand()*(32767 + 1) + rand();
        return ((float)rnd / (32767*32767) * (max - min)) + min;
#else
        return ((float)rand() / 32767 * (max - min)) + min;
#endif
...
#else
    return ((float)random_gen() / (float)32767);
#endif
}
