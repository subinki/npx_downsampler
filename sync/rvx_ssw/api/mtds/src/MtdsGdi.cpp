...
**	Parameters:
**		hds		- handle to the drawing state object
**		ity		- oled_rgb_draw_bitmap intensity to set
**
**	Return Values:
**		none
**
...
**
**	Description:
**		Set the specified value as the intensity to use for oled_rgb_draw_bitmap(). This
...
**
**	Return Values:
**		Returns the current oled_rgb_draw_bitmap intensity
**
**	Errors:
**		Returns 0 if error occurs, which happens to be the color black
**
**	Description:
**		Return the current intensity used for oled_rgb_draw_bitmap().
*/
...
/* ------------------------------------------------------------ */
/***	MTDS::oled_rgb_draw_bitmap(hdsDst, xcoDst, ycoDst, dxco, dyco, hdsSrc, xcoSrc, ycoSrc)
**
**	Parameters:
...
bool MTDS::oled_rgb_draw_bitmap(HDS hdsDst, int16_t xcoDst, int16_t ycoDst, int16_t dxco, int16_t dyco, HDS hdsSrc, int16_t xcoSrc, int16_t ycoSrc) {
	PRM1A4B1A2B prm;
	...
	*/
	...
	MtdsProcessCmdWr(clsCmdGdi, cmdGdioled_rgb_draw_bitmap, sizeof(prm), (uint8_t *)&prm, 0, 0);
